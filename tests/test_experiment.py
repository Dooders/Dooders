...
class TestExperiment:
    ...
    def test_simulate(self, experiment):
        with patch.object(experiment, 'create_simulation') as mock_create_simulation, \
            ...
            mock_simulation.run_simulation.assert_called_once_with(experiment.batch, 1)
    ...
    def test_save_object(self, experiment):
        with patch('builtins.open', new_callable=unittest.mock.mock_open()) as mock_open, \
            ...
            mock_exists.assert_called_once_with(f'experiments/{experiment.save_folder}/')
            mock_makedirs.assert_called_once_with(f'experiments/{experiment.save_folder}/')
            mock_open.assert_called_once_with(save_path, 'w')
            mock_dump.assert_called_once_with(object_to_save, mock_open.return_value.__enter__.return_value)  # Corrected line
